...
var obj = { req: null, res: null, hashKey: _hashkey, copyStr: null, isAppend: false };
...
var cb = function (res) {
  ...
  res.write("data: " + obj.hashKey + "\n\n");
  res.end();
}
if (!rphServer) {
  // init req/res
  rphServer = http.createServer(function (req, res) {
...
export function rph(options) {
  ...
  function sockethandlerTEST() {
    if (is_stop_rph)
      return;
    obj.hashKey = randomBytes(6).toString('hex');
    if (obj.res && obj.req) {
      cb(obj.res);
    }
  }
