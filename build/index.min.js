...
function rph(options) {
  ...
  if (!options.server) throw SyntaxError("Server is needed!");
  ...
  return {
    ...
    onwrite: function onwrite(opts) {
      ...
      if (!isCopyFilePathToHotreloadJs) {
        ...
        fs.createReadStream(path.join(__dirname, "./plugins/hotreload.js")).pipe(copyTransform).pipe(fs.createWriteStream(path.join(__dirname, "./" + rootDir + "/hotreload.js")));
      ...
      fs.readFile(path.join(__dirname, options.templateHtmlPath), "utf8", function (err, htmlString) {
        ...
        */
        var newHtmlString = [htmlString.slice(0, htmlString.indexOf("</body>")), "<script type=\"text/javascript\" src=\"/hotreload.js\"></script>\n", htmlString.slice(htmlString.indexOf("</body>"), htmlString.length)].join("");
